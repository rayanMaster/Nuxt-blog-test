...
import * as utils from '~/composables/useGetPostsList'
...
describe('fetchData', () => {
    ...
    const mockedImplementation = () => Promise.resolve({
        json() {
            return {posts: mockPostsData, pending: false}
        }
    })
    ...
    it('should return the mocked data', async () => {
        ...
        const spyOnUseGetPostsList = vi.spyOn(utils, 'useGetPostsList');
        spyOnUseGetPostsList.mockImplementation(mockedImplementation);

        const response = await utils.useGetPostsList(postListRequest, order, loadNewData);
        const {posts,pending} = response.json()
